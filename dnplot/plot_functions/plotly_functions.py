...
def waveseries_plotter_basic(model):
    ...
    for var in ts.core.data_vars():
        trace = go.Scatter(
            x=ts.time(),
            y=ts.get(var),
            mode="lines",
            name=f"{var} ({ts.meta.get(var).get('unit')})",
            visible="legendonly",
        )
        fig.add_trace(trace)
...
def waveseries_plotter_dash(model):
    ts = model.waveseries()
    var = xarray_to_dataframe(ts.ds())
    options = [
        {"label": f"{var} ({ts.meta.get(var).get('unit')})", "value": var}
        for var in ts.core.data_vars()
    ]
    app = Dash(__name__)
    app.layout = html.Div(
...
def scatter_plotter(model, model1):
    ...
    name1 = model.waveseries().name
    name2 = model1.waveseries().name
    var1 = model.waveseries().core.data_vars()[0]
    var2 = model.waveseries().core.data_vars()[0]

    ws = model.waveseries().ds().dropna("time")
    ws2 = model1.waveseries().ds().dropna("time")

    common_times = (ws.get(var1) - ws2.get(var2)).time.values

    ws = ws.sel(time=common_times)
    ws2 = ws2.sel(time=common_times)

    # df_model = xarray_to_dataframe(model.waveseries().ds())
    # df1_model1 = xarray_to_dataframe(model1.waveseries().ds())
    ...
    app = Dash(__name__)
    options_x = [
        {"value": col, "label": f"{col}_{name1}"} for col in ws.data_vars
    ]
    options_y = [
        {"value": col, "label": f"{col}_{name2}"} for col in ws2.data_vars
    ]
    app.layout = html.Div(
        [
    ...
    def update_graph(x_var, y_var):
        ...
        # x = df_noNa[x_col].values
        # y = df_noNa[y_col].values
        x = ws.get(x_var)
        y = ws2.get(y_var)
        xy = np.vstack([x, y])
        z = gaussian_kde(xy)(xy)
        fig = px.scatter(x=x, y=y, color=z, color_continuous_scale="jet")
        # fig = px.scatter(
