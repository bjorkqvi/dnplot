...
def plot_gridded_magnitude(fig_dict, x, y, data, cmap, vmax = None, vmin=None, label: str='__nolegend__'):
    """
    ...
    # if vmax-vmin<20:
    levels = np.linspace(np.floor(vmin), np.ceil(vmax), np.floor(vmax-vmin+2).astype(int))
    # else:
...
def plot_masked_points(fig_dict, grid, masks_to_plot: list[str], default_dict: dict=default_markers.get('generic_masks')):
    ...
    for mask in masks_to_plot:
        ...
        fig_dict.get('ax').plot(x,y,markers.get('marker')+markers.get('color'), markersize=markers.get('size'), label='Set '+ mask.split('_')[0] + ' points')

    return fig_dict

def plot_object_points(fig_dict, model, objects_to_plot: list[str], default_dict: dict=default_markers.get('generic_objects')):
    """Plots points og objects to the plot"""
    for obj_type in objects_to_plot:
        markers = dict(default_dict)
        markers.update(default_markers.get(obj_type.value, default_dict))

        if model[obj_type] is not None:
            x, y = model[obj_type].xy(native=True)
            fig_dict.get('ax').plot(x,y,markers.get('marker')+markers.get('color'), markersize=markers.get('size'), label='Imported ' + obj_type.value)

    return fig_dict

def plot_mask(fig_dict, x_vec,y_vec, mask) -> dict:
...
def plot_arrows(fig_dict, lon, lat, xdata, ydata, scale=100, reduce_arrows: int=None):
    """
...
def plot_polar_spectra(fig_dict, spec, freq, dirs) -> dict:
    ...
    cont = ax.contourf(np.deg2rad(dir_plot), freq, spec_plot, cmap="ocean_r", levels=levels)
    fig_dict['cont'] = cont
